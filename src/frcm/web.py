...
<head>
    ...
    <style>
        ...
        input[type="number"], input[type="text"] { padding: 8px; width: 100%; font-size: 14px; margin: 5px 0; box-sizing: border-box; }
        button {
            padding: 10px 20px;
...
<body>
    ...
    <script>
        ...
        async function fetchWeather() {
            ...
            try {
                let url = `/api/weather/latest?latitude=${encodeURIComponent(lat)}&longitude=${encodeURIComponent(lon)}`;
                if (alt) {
                ...
                if (!response.ok) {
                    throw new Error(data.detail || 'Feil ved henting av værdata');
                }

                const weather = data.weather;

                document.getElementById('result-time').textContent =
                    new Date(weather.timestamp).toLocaleString('no-NO');
                ...
                document.getElementById('result-humidity').textContent =
                    Number(weather.humidity).toFixed(1);
                document.getElementById('result-wind').textContent =
                    Number(weather.wind_speed).toFixed(1);

                resultDiv.classList.remove('error');
                resultDiv.classList.add('show');
            } catch (error) {
                resultDiv.innerHTML = `<p><strong>Feil:</strong> ${error.message}</p>`;
...
async def get_weather_latest(
    ...
    try:
        ...
        logger.exception("Unexpected error while fetching latest weather")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
